class Chronometer {
  ...
  getMilliseconds() {
    ...
    return Math.floor(time.toFixed(2) * 100); //used Math.floor and toFixed to correct rounding errors
  }
  ...
  splitClick(minutes, seconds) {
    return `${this.twoDigitsNumber(this.getMinutes(minutes))}:${this.twoDigitsNumber(this.getSeconds(seconds))}:${this.twoDigitsNumber(this.getMilliseconds())}`;
  }
}